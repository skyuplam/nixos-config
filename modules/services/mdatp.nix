...
in {
  ...
  config = mkIf cfg.enable {
    ...
    systemd = {
      ...
      services.mdatp = {
        ...
        serviceConfig = {
          Type = "simple";
          #ExecStartPre = "${cfg.package}/bin/mdatp_pre_start";
          ExecStart = "${cfg.package}/bin/mdatp_daemon --log-level ${cfg.logLevel}";
      ...
      services.mdatp-netfilter = mkIf cfg.enableNetworkProtection {
        ...
        serviceConfig = {
          ...
          ExecStart = "${cfg.package}/bin/mde_netfilter";
          NotifyAccess = "main";
          LimitCORE = "infinity";
          KillMode = "process";
          Restart = "on-failure";
      ...
      sockets.mdatp-netfilter = mkIf cfg.enableNetworkProtection {
        description = "Microsoft Defender Netfilter Activation Socket";
        listenStreams = ["/run/mde_netfilter.sock"];
  ...
  meta = {
    maintainers = with lib.maintainers; [skyuplam];
  };
}
